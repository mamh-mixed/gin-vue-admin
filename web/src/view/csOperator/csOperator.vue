<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="分润方式" prop="allocation" width="120">
    ...
    <el-dialog v-model="detailShow" style="width: 800px" lock-scroll :before-close="closeDetailShow" title="查看详情" destroy-on-close>
      <el-scrollbar height="550px">
        <el-descriptions column="1" border>
                ...
                <el-descriptions-item label="到期字段">
                    {{ formData.endTime }}
                </el-descriptions-item>
                <el-descriptions-item label="分润方式">
                  {{ filterDict(formData.allocation,allocationOptions) }}
                </el-descriptions-item>
...
const getTableData = async() => {
  const table = await getCsOperatorList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteCsOperatorByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCsOperatorFunc = async(row) => {
    const res = await findCsOperator({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCsOperatorFunc = async (row) => {
    const res = await deleteCsOperator({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
